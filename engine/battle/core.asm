...
.calcCriticalHitProbability
	...
	jr nz, .focusEnergyUsed      ; fixed: focus energy used to shift right instead of left,
	                             ; resulting in 1/4 the usual crit chance instead of 4x
	sla b                        ; (effective (base speed/2)*2)
	jr nc, .noFocusEnergyUsed
	ld b, $ff                    ; cap at 255/256
	jr .noFocusEnergyUsed
.focusEnergyUsed
	sla b                        ; (effective (base speed/2)*8, i.e. 4x the usual crit chance)
	jr c, .maxCritChance
	sla b
	jr c, .maxCritChance
	sla b
...
; fixed: the maximum accuracy used to be a 255/256 chance instead of 100%,
; so a fully accurate move could still randomly miss
	ld a, b
	inc a
	ret z ; never miss if the scaled accuracy is at its maximum
	call BattleRandom
	cp b
	jr nc, .moveMissed
	ret
